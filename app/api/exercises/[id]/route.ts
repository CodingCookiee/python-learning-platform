 ...
 * Get exercise details including user submission history
 * Cached for 1 hour per user
 */
export const GET = withAuth(async (req: NextRequest, context: AuthContext<{ id: string }>) => {
  try {
    const { id } = await context.params;
    const cacheKey = CacheKeys.exercise(id, context.userId);
    ...
    const response = await getCached(
      ...
      async () => {
        ...
        // Don't return solution unless user has solved it or made 3+ attempts
