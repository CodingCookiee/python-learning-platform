...
def _():
    output = run_program(stdin=["12", "23", "twelve", "9", "13", ""])
    assert output.lines == [
...
@test('Stops at "done"')
def _():
    output = run_program(stdin=["5", "8", "done", "100"])
    assert output.lines == ["Days: 2", "Total: 13", "Best day: day 2 (8 units)"]
...
def _():
    output = run_program(stdin=["4", "11", "6", "11", ""])
    assert output.lines[-1] == "Best day: day 2 (11 units)"


@hidden('Accepts "DONE" in capitals and numbers with spaces around them')
def _():
    output = run_program(stdin=[" 7 ", "3", "DONE"])
    assert output.lines == ["Days: 2", "Total: 10", "Best day: day 1 (7 units)"]


@hidden("Counts only valid lines as days")
def _():
    output = run_program(stdin=["-3", "2.5", "abc", ""])
    assert output.lines == [
